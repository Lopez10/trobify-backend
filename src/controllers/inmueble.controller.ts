...
export async function registerInmueble(req: Request, res: Response): Promise<Response> {
	...
	if (reggy != ' ') {
		...
		const usuario = conn2.query(
			'SELECT id_usuario FROM Catalogo WHERE id_catastro = "' + id_catastro + '";'
		);
		...
		for (var i = 0; i < imagenes.toString().length; i++) {
			...
			let valImagen: string = 'VALUES (' + id_catastro + ', ' + imagenes.toString().splice + ');';
		}

		let insertinto2: string = ' INSERT INTO Catalogo ';
		let val2: string =
			'VALUES (' + id_catastro + ', ' + id_modalidad + ', ' + precio + ', 0, 0, ' + usuario + ');';
		/*
...
*/
		...
		let val4: string =
			'VALUES (' + id_catastro + ', ' + nBano + ', 1, ' + id_certifEner + ', ' + nHab + ');';

		conn2.query(insertinto + values);
		conn2.query(insertinto2 + val2);
		for (var i = 0; i < caracteristica.toString().length; i++) {
			let insertinto3: string = ' INSERT INTO Contiene ';
			let val3: string = 'VALUES (' + caracteristica.toString().split + ', ' + id_catastro + ');';
...
*/
		...
		for (var i = 0; i < caracteristica.toString().length; i++) {
			let inse2: string = ' INSERT INTO Contiene CO';
			let values2: string = ' VALUES (' + caracteristica.toString().split + ', ' + catast + ');';
