...
export class ConsultaSedeCatastral {
	id_catastro: string; //
	direccion: string;
	localidad: string; //
	codigoPostal: number;
	...
	static async create(id_catastro: string, SRS?: number): Promise<ConsultaSedeCatastral> {
		const catastro = new ConsultaSedeCatastral();
		catastro.id_catastro = id_catastro;

		const ubicacionCatastro = await ConsultaSedeCatastral.consultaCatastroUbicacion(id_catastro);
		catastro.coordenada = {
		...
		const otrosDatosCatastro = await ConsultaSedeCatastral.consultaCatastroDatosInmueble(
			id_catastro
		);
	...
	static async consultaCatastroUbicacion(id_catastro: string, SRS?: number): Promise<string[]> {
		...
		httpGet += '&SRS=' + ConsultaSedeCatastral.codigoSRS();
		httpGet += '&RC=' + ConsultaSedeCatastral.calculaRC(id_catastro);
