...
export async function getCatalog1(req: Request, res: Response): Promise<Response> {
	...
	let select: string =
		'inm.id_catastro, inm.superficie, inm.breveDescripcion, ubi.direccion, ubi.latitud, ubi.longitud, ubi.prov, cat.precio, cat.descuento, cat.id_usuario as propietario, car.nHab, car.nBano, car.nCocina, tpoI.tipoInmueble, tpoV.tipoVivienda, est.estadoInmueble, cer.certifEner';
	let from: String =
	...
	where += ' AND ubi.prov = ' + idProvincia;
	where += ' AND tpoI.id_tipoInmueble = ' + idTipoInmueble;

	//console.log('SELECT ' + select + ' FROM ' + from + ' WHERE ' + where + ';');

	const conn = await connect();
	const catalogo = await conn.query('SELECT ' + select + ' FROM ' + from + ' WHERE ' + where + ';');

	return res.json(catalogo[0]);
}

function sonSuficientesCriterios(
	idProvincia: Number,
	idTipoInmueble: Number,
	idModalidad: Number
): boolean {
	...
	if (isNaN(+idTipoInmueble) || idTipoInmueble < 0 || idTipoInmueble > 52) {
		return false;
	}

	if (isNaN(+idModalidad) || idModalidad < 0 || idModalidad > 52) {
		return false;
	}
